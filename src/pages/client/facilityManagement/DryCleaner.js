...
export default function DryCleaner({ facility }) {
  ...
  useEffect(() => {
    const savedStartTime = localStorage.getItem(
      `cleaner-startTime-${facility.facilityId}`
    );
    const savedDuration = localStorage.getItem(
      `cleaner-duration-${facility.facilityId}`
    );
    const savedMaxTime = localStorage.getItem(`cleaner-maxTime-${facility.facilityId}`);
      ...
      timer = setInterval(() => {
        setProgress((prev) => {
          ...
          if (newProgress >= 100) {
            ...
            localStorage.removeItem(`cleaner-startTime-${facility}`);
  ...
  const handleLaundryWayChange = (e) => {
    ...
    localStorage.setItem(`cleaner-laundryWay-${facility.facilityId}`, e.target.value);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ border: "0.16rem solid #FF8C00" }}
      >
        ...
        {facility.facilityStatus === "F" ? (
          ...
          <button
            className="Facility-button"
            style={{ backgroundColor: "#FF4500", width: "50%" }}
